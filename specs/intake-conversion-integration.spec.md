...
1. `checks/intake-convert.mjs` (`npm run intake:convert -- <source> [out.md]`) converts
   ...
   `python -m markitdown` / `python3 -m markitdown`). If none is found it graceful-skips (exit 0)
...
3. Markdown stays the source of truth (`AGENTS.md`): the converted `.md` is normalized
   ...
   not auto-install markitdown and does not invoke transient package runners such as
   `uvx` or `pipx run`.
...
- markitdown's MCP server and Azure/OCR plugins are out of scope (online/heavy); only
  the already-installed local CLI / Python module conversion is referenced.
...
- `tests/intake-conversion.test.mjs` — asserts the helper exists and exports the pure
  `buildOutPath` (correct `.docx`→`.md` mapping), converter resolution without auto-install, absent-tool graceful-skip behavior, that no markitdown/python dependency was added to `package.json`, that the converter is absent from the `check` chain, and that the owners document the optional step. Run: `node tests/intake-conversion.test.mjs`.
